 ...
 *
 * Typically, these are lists of keys that are defined at runtime, multiple lists are allowed in the case that there are
 * different environments with different keys that are set, and the compiler will fail unless keys are available for
 * *all* environments.
 */
class ResolveLists(roles: Map[String, Set[String]]) {
  ...
  /**
   * Given a set of keys, return a mapping from list name to the set of those keys that are missing for that list.
   */
